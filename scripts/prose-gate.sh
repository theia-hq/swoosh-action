...
PROCESS='delib-[0-9]|deliberation [0-9]|notes/reviews/|notes/deliberations/|SYNTHESIS|round-[0-9]|FLAG\([A-Za-z]'
...
SEATS='(Rust Reviewer|Style Warden|Founder.s Advocate|Systems Architect|CLI Architect|Product Lead|the Adversary|the Cartographer|the Convener|the Librarian|the Skeptic|the Scribe|the Editor|the Newcomer|the Operator|the Craftsman|the Principal|the Visionary|the Builder)'
...
for f in $files; do
    [ -f "$f" ] || continue
    found=$(grep -nE "$PROCESS|$SEATS" "$f" 2>/dev/null || true)
    [ -n "$found" ] || continue
    printf 'PROCESS    %s\n' "$f"
    printf '%s\n' "$found" | sed 's/^/        /'
    hits=$((hits + $(printf '%s\n' "$found" | grep -c .)))
