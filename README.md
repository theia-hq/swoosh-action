...
**The name.** *swoosh-ssh* is the piece that lets you `swoosh ssh` into a CI runner: `swoosh` is the
command you dial with, `ssh` the shell you land in.
...
- **Keyless, but gated by membership.** The shell (`sshd:`) has no auth of its own, so it is exposed
  ...
  swoosh refuses to expose `sshd:` with `--public` for exactly this reason.
- **Log-safety by design.** `swoosh tunnel expose --quiet`, so the NodeId is never printed; it can't leak
...
- **Checksum-verified install.** The `swoosh` binary is downloaded from its GitHub
